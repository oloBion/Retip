...
fit.lightgbm <- function(training,testing) {
  ...
  dtest <- lightgbm::lgb.Dataset.create.valid(dtrain, test[,2:coltest],
                                              label = test[, 1])
  ...
  modelcv <- lightgbm::lgb.cv(params, dtrain, nrounds = 5000, nfold = 10,
                              valids, verbose = 1,
                              early_stopping_rounds = 1000, record = TRUE,
                              eval_freq = 1L, stratified = TRUE)
